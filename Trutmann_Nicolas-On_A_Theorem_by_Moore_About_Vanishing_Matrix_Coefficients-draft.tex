...
\section{The Direct Integral and Unitary Representations}\label{the-direct-integral-and-unitary-representations}}
  ...
  % TODO: Corr: big whopsie here. not sure how to tackle
...
\section{Proving the Theorem}\label{proof-for-sl2r}}
  ...
  % TODO: lots of comments on this one. 
  \begin{proof}
    \label{pf:lemma}
    ...
    % TODO: Corr: k and k' are not defined
    \begin{align*}
      &\ \abs{\inn{\pi(a_n)\pi(k_n)v, \pi(k_n)^{-1}w}  -  \inn{\pi(a_n)\pi(k)v, \pi(k')w}} \quad \text{(now ommiting $\pi$)} \\
  ...
  \begin{proof}
    ...
    theorem \thref{thm:2.3.6} and the preceeding lemma \thref{lemma} does the bulk of
    ...
    % TODO: Corr: here is a reference to a figure that's not here anymore
    for $b=0$ every individual point $(a\ 0)$ on the $x$-axix is an orbit. (See Figure~\ref{fig:n-orbits-in-r2}). As $f$ is \Ninv,
...
\label{proof-for-slnr}}
  ...
  % TODO: Corr. this is sort of wrong.
...
\section{Conclusion}\label{outro}}
  ...
  \begin{thm}[Zimmer 2.2.15]
    ...
    % TODO: Corr: haven't defined irreducible
  ...
  Recall that the defition of a lattice specifies that the quotient $G/\Gamma$
